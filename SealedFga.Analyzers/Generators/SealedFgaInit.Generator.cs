...
internal static class SealedFgaInitGenerator {
        ...
        var idUtilInitializations = idClasses
           .SelectMany(idClass => {
                    ...
                    return new List<string>([
                            ...
                            // A lambda (not a method group): Parse returns a struct, and covariant
                            // method-group conversion to Func<string, object> is illegal for value
                            // types — the lambda boxes the result instead.
                            $"IdUtil.RegisterIdTypeParseMethod(typeof({fullClassName}), val => {fullClassName}.Parse(val));",
                        ]
                    );
                }
            );
