...
public class MiddlewareTests {
    ...
    [Fact]
    public async Task Unauthenticated_exception_maps_to_401() {
        var ctx = await InvokeWithThrow(new FgaUnauthenticatedException("open_fga_user"));
        ctx.Response.StatusCode.ShouldBe((int) HttpStatusCode.Unauthorized);
    }
