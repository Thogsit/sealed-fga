...
public class SaveChangesProcessorTests {
    ...
    [Fact]
    public void Changed_primary_key_emits_single_modify_id_and_skips_per_property() {
        var (ctx, conn) = TestDbContext.CreateSqlite();
        using var _ = conn;

        var oldId = TestObjectId.New();
        var obj = new TestReassignableEntity { Pk = 1, Id = oldId, ParentId = new TestParentId(Guid.NewGuid()) };
        ctx.Reassignables.Add(obj);
        ctx.SaveChanges();

        // Change BOTH the SealedFGA Id and the relation FK. The processor must emit a single ModifyId
        // row and skip per-property (relation) emission entirely.
        var newId = TestObjectId.New();
        obj.Id = newId;
        obj.ParentId = new TestParentId(Guid.NewGuid());
        ...
        rows.Count.ShouldBe(1);
        var row = rows[0];
        row.OperationType.ShouldBe(SealedFgaOutboxOperationType.ModifyId);
        row.TargetId.ShouldBe(oldId.AsOpenFgaIdTupleString());
        row.NewTargetId.ShouldBe(newId.AsOpenFgaIdTupleString());
        row.TypeName.ShouldBe(TestObjectId.OpenFgaTypeName);
    }
