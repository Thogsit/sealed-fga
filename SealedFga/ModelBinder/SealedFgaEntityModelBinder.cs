...
public class SealedFgaEntityModelBinder(Type dbContextType)
    ...
    protected override async Task FgaBind(
        ...
        if (parameterVal == null) {
            // If the action executed, a real route parameter always carries a value — null means
            // ParameterName does not name a route parameter (typo, or the id comes from query/body).
            throw new InvalidOperationException(
                $"[FgaAuthorize] on parameter '{param.Name}' of action "
                + $"'{context.ActionContext.ActionDescriptor.DisplayName}': ParameterName '{attr.ParameterName}' "
                + "does not match any route value. It must name the route parameter that carries the entity ID."
            );
        }
        ...
        if (idType == null) {
            throw new InvalidOperationException(
                $"[FgaAuthorize] on parameter '{param.Name}' of action "
                + $"'{context.ActionContext.ActionDescriptor.DisplayName}': ParameterName '{attr.ParameterName}' "
                + "does not name an action parameter."
            );
        }
        ...
        var openFgaIdTupleStringMethod =
            SealedFgaBinderReflectionCache.OpenFgaIdTupleStringMethod(idType.ParameterInfo.ParameterType);
        var openFgaRawObjectString = (string?) openFgaIdTupleStringMethod.Invoke(parsedId, null);
        if (openFgaRawObjectString == null) {
            throw new InvalidOperationException(
                $"'{GeneratedNames.OpenFgaIdTupleStringMethodName}' on ID type "
                + $"'{idType.ParameterInfo.ParameterType.Name}' returned null for value '{parameterVal}'."
            );
        }

        // Resolve the optional per-call options hook (contextual tuples, consistency). The object
        // type name is the prefix of the entity's tuple string ("secret:<id>" -> "secret").
        var objectTypeName = openFgaRawObjectString[..openFgaRawObjectString.IndexOf(':')];
        var queryOptions = await GetBinderQueryOptionsAsync(
            context,
            openFgaRawUserString,
            attr.Relation,
            objectTypeName,
            SealedFgaBinderOperation.Check
        );
        ...
        var setMethod = SealedFgaBinderReflectionCache.Set(entityType);
    ...
    private static async Task<object?> FindByPrimaryKeyAsync(object dbSet, object parsedId) {
        var findAsyncMethod = SealedFgaBinderReflectionCache.FindAsync(dbSet.GetType());
    ...
    private static async Task<object?> LoadWithIncludesAsync(
        ...
        var firstOrDefaultMethod = SealedFgaBinderReflectionCache.FirstOrDefaultAsync(entityType);
